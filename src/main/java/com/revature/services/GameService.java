...
public class GameService {
	...
	private int getResult(String gameState) { //returns 0 for in progress, 1 for player x win, 2 for player o win, 3 for draw, 4 for invalid game
		String[] strs = gameState.split("");
		...
		//rows
			if(getTriadResult(state[0], state[1], state[2])) {count++;}
			if(getTriadResult(state[3], state[4], state[5])) {count++;}
			if(getTriadResult(state[6], state[7], state[8])) {count++;}
		
		//collums
			if(getTriadResult(state[0], state[3], state[6])) {count++;}
			if(getTriadResult(state[1], state[4], state[7])) {count++;}
			if(getTriadResult(state[2], state[5], state[8])) {count++;}
		
		//diagonals
			if(getTriadResult(state[0], state[4], state[8])) {count++;}
			if(getTriadResult(state[2], state[4], state[6])) {count++;}
